...
The image tags can be seen at https://hub.docker.com/repository/docker/pawapay/activemq-artemis

## 3. About this image

The ActiveMQ Artemis image is

- Debian based
- Based on openjdk17
- Supports amd64 & arm64 platforms
...
To run ActiveMQ with AMQP, JMS and the web console open,
...
In some circumstances it might be advisable to fine tune the memory to manual values, in that case you can set the memory that you application needs by using the parameters `ARTEMIS_MIN_MEMORY` and `ARTEMIS_MAX_MEMORY`:
...
You can see the [helm chart](https://github.com/pawapay/helm-charts) for Kubernetes and some configuration tuning for OpenShift.
